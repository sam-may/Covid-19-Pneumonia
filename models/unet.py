...
def std_conv(name, input_img, n_layers, n_filters, kernel_size, max_pool= 3,
    ...
    for i in range(n_layers):
        ...
        if batch_norm:
            conv = keras.layers.BatchNormalization(axis = 1)(conv)
    ...
    if max_pool >= 4:
        conv = keras.layers.MaxPooling3D(
            pool_size = (max_pool,max_pool,max_pool),
...
def up_conv(name, input_img, n_layers, n_filters, kernel_size, aux_image = None,
    ...
    if aux_image is not None:
        conv = keras.layers.Concatenate()([conv, aux_image],axis = 5)

    return conv

def unet3D(config, verbose = True):
    # Unpack config
