...
def cnn(n_pixels, config):
    ...
    output = keras.layers.Dense(1, activation = 'sigmoid', kernel_initializer = 'lecun_uniform', name = 'output')(conv)
...
def std_conv(name, input_img, n_layers, n_filters, kernel_size, max_pool = 2, dropout = 0.25, batch_norm = True, activation = 'elu', conv_dict = {}):
    ...
    for i in range(n_layers):
        ...
        if dropout > 0:
            conv = keras.layers.Dropout(dropout, name = 'std_conv_dropout_%s_%d' % (name, i))(conv)
...
def up_conv(name, input_img, n_layers, n_filters, kernel_size, aux_image = None, dropout = 0.25, batch_norm = False):
    conv = input_img
    ...
    for i in range(n_layers):
        ...
        if dropout > 0:
            conv = keras.layers.Dropout(dropout, name = 'up_conv_dropout_%s_%d' % (name, i))(conv)
