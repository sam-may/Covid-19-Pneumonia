...
class Data_Helper():
    ...
    def assess(self, patient, N, tag): # make plots of N randomly chosen images
        if N > 0:
            for i in range(N):
                ...
                output_img = self.output_dir + "/images/image_and_truth_%s_%s_%d.pdf" % (tag, patient, i)
    ...
    def z_score(self, patient):
        if not ("mean" in self.metadata["preprocess_scheme"].keys() or "std" in self.metadata["preprocess_scheme"].keys()):
    ...
    def calculate_mean_and_std(self):
        ...
        return numpy.mean(pixel_values.astype(numpy.float64)), numpy.std(pixel_values.astype(numpy.float64))
    ...
    def write(self, patient):
        ...
        for X, y in zip(self.patients[patient]["X"], self.patients[patient]["y"]):
            ...
            self.metadata[patient].append( { "X" : dset_X, "y" : dset_y, "n_pneumonia" : n_pneumonia } ) 
